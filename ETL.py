...
class ETL_class(object):
    ...
    def __init__(self, path_titles: str):
...
#     def get_movies(self, platform: str):
#         df = pd.read_csv(self.path_title + platform)

#         df_rating = self.get_ratings()
#         df_rating = df_rating.groupby(['movieId']).agg({'mean'}).reset_index()

#         merge = df.merge(df_rating, left_on='id', right_on='movieId', how='inner')
#         old_names = merge.columns
#         new_names = ['id', 'title', 'cast', 'release_year', 'duration_int', 'duration_type', 'movieId', 'rating']
#         merge.rename(columns={o: n for (o, n) in zip(old_names, new_names)}, inplace=True)
#         merge.drop(columns=['movieId'], inplace=True)
#         merge.to_csv(self.path_title + 'merges/' + platform, index=False)
#         return df.shape, merge.shape
...
#     print(etl.get_movies(t))

# titles = etl.get_csv_files(path_titles)
# rating = etl.get_csv_files(path_rating)

# for t in titles:
#     df_t = etl.etl_movies(t)[['id', 'title', 'cast', 'release_year', 'duration_int', 'duration_type']]
